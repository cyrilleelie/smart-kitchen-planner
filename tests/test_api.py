...
def test_submit_feedback(client, mock_db_session):
    ...
    response = client.post(
        "/feedback", json={"user_id": 1, "recipe_id": 100, "rating": 5}
    )
    assert response.status_code == 200
...
def test_contextual_recommendation(client, mock_db_session):
    ...
    with patch(
        "src.recommender.inference_service.InferenceService"
    ) as MockServiceClass:
        ...
        mock_instance.recommend.return_value = [
            {"id": 200, "name": "Salad", "score": 0.88, "type": "AI"}
        ]

        response = client.post(
            "/recommend", json={"user_id": 1, "meal_type": 1, "season": 2}
        )
