...
def test_calculate_context_penalty():
    ...
    calculate_context_penalty(mock_recipe, meal_type=2, season=0)
...
def test_train_rf_success(mock_session, mock_mlflow):
    ...
    with patch("src.mlops.train_model.engine"), patch(
        "src.mlops.train_model.os.makedirs"
...
def test_train_svd_success(mock_engine, mock_session, mock_mlflow):
    ...
    with patch("src.mlops.train_model.SVD") as mock_svd_cls, patch(
        "src.mlops.train_model.Dataset"
    ), patch("src.mlops.train_model.surprise_train_test_split") as mock_tts, patch(
        "src.mlops.train_model.surprise.dump.dump"
    ) as mock_dump:
...
def test_optimize_svd_run(mock_sess_cls, mock_engine, mock_surprise_cv):
    ...
    mock_sess.query.return_value.join.return_value.all.return_value = [
        (mock_int, mock_rec)
    ]
