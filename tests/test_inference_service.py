...
def test_recommend_flow(mock_service):
    ...
    with patch("src.recommender.inference_service.UserProfiler") as MockProfiler:
        ...
        mock_profiler.get_weighted_profile.return_value = np.array(
...
def test_recommend_weekly_batch(mock_service):
    ...
    candidates = [
        Mock(id=i, embedding="[0.1, 0.1]", nutrition_info="['500']") for i in range(50)
    ]

    mock_service.get_user_vector = Mock(
    ...
    plan = mock_service.recommend_weekly_batch(
        1, 1, 0, mock_session, n_days=5, target_calories=2000
    )
