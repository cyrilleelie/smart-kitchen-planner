...
def test_get_converted_tags_nominal(mock_profiler):
    ...
    with patch.dict(src.utils.translations.PREFERENCE_TAGS_MAP, MOCK_TAGS_MAP, clear=True):
...
def test_get_weighted_profile_full_flow(mock_or, mock_profiler):
    ...
    def query_side_effect(model):
        ...
        if 'Recipe' in model_name:
            m.filter.return_value.filter.return_value.limit.return_value.all.return_value = [fixed_recipe]
