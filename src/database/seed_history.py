...
def seed_interactions():
    ...
    with Session(engine) as session:
        ...
        recipes = session.query(Recipe).filter(Recipe.name.ilike('%chicken%')).limit(10).all()
        ...
        for recipe in all_recipes:
            ...
            exists = session.query(Interaction).filter_by(user_id=1, recipe_id=recipe.id).first()
