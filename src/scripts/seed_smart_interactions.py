...
def parse_list(str_value):
    ...
    try:
        ...
        if isinstance(str_value, list): return str_value
...
def get_simulated_score(user_id, recipe):
    ...
    if user_id == 1:
        # Bonus
        if "vegetarian" in tags_str or "vegan" in tags_str: score += 1.5
        if "salad" in tags_str or "healthy" in tags_str: score += 1.0
        if recipe.calories and recipe.calories < 400: score += 0.5
        
        # Malus (Repoussoirs forts)
        for meat in ["chicken", "beef", "pork", "bacon", "steak", "sausage"]:
            if meat in ing_str: score -= 3.0
        if "deep-fried" in tags_str: score -= 1.0

    # --- 2. LE TRADITIONNEL (VIANDE & PATATES) ---
    elif user_id == 2:
        # Bonus
        if any(x in ing_str for x in ["beef", "steak", "pork", "roast", "meat"]): score += 1.5
        if recipe.minutes > 60: score += 1.0 # Aime le mijoté
        if "comfort-food" in tags_str: score += 1.0
        
        # Malus
        if "tofu" in ing_str or "vegan" in tags_str: score -= 2.5
        if "salad" in name_str: score -= 1.0

    # --- 3. L'EXPLORATEUR ÉPICÉ (WORLD FOOD) ---
    elif user_id == 3:
        # Bonus
        spicy_keywords = ["spicy", "curry", "chili", "sriracha", "cajun", "mexican", "indian", "thai", "asian"]
        if any(k in tags_str or k in name_str for k in spicy_keywords): score += 2.0
        if "rice" in ing_str: score += 0.5
        
        # Malus
        if "bland" in tags_str or "simple" in tags_str: score -= 1.0
        if "european" in tags_str and "spicy" not in tags_str: score -= 0.5

    # --- 4. LE FIT & SPORT (PROTEIN & CONTROL) ---
    elif user_id == 4:
        # Bonus (Cherche le poulet/poisson sans trop de gras)
        if "chicken" in ing_str or "fish" in ing_str or "tuna" in ing_str or "egg" in ing_str: score += 1.5
        if "high-protein" in tags_str: score += 1.5
        
        # Malus (Trop calorique ou sucre)
        if recipe.calories and recipe.calories > 800: score -= 2.0
        if "dessert" in tags_str or "cake" in tags_str or "chocolate" in ing_str: score -= 3.0

    # --- 5. LE BEC SUCRÉ (DESSERT ONLY) ---
    elif user_id == 5:
        # Bonus
        if "dessert" in tags_str or "cookie" in tags_str or "cake" in tags_str: score += 2.5
        if "chocolate" in ing_str or "sugar" in ing_str or "fruit" in tags_str: score += 1.0
        
        # Malus
        if "main-dish" in tags_str or "dinner-party" in tags_str: score -= 1.5
        if "onion" in ing_str or "garlic" in ing_str: score -= 2.0
...
def seed_interactions():
    ...
    with Session(engine) as session:
        ...
        print(f"   🎲 Simulation de {NB_RECIPES_TO_RATE} notes/user sur un pool de {len(recipes)} recettes...")
