...
def add_interactions(username: str, count: int, persona_path: str):
    print(
        f"🚀 Adding {count} new interactions for user '{username}' using rules from '{persona_path}'"
    )
    ...
    with Session(engine) as session:
        ...
        rated_recipe_ids = {
            i.recipe_id
            for i in session.query(Interaction.recipe_id)
        ...
        print(
            f"   🎲 Selected {sample_size} new recipes to rate (out of {len(candidate_ids)} available)."
        )
        ...
        selected_recipes = (
            session.query(Recipe).filter(Recipe.id.in_(selected_ids)).all()
        )
        ...
        print(
            f"✅ Successfully added {len(new_interactions)} new interactions for '{username}'."
        )
