...
class MenuSolver:
    ...
    def solve(self) -> list[dict[str, Any]]:
        ...
        """
        logger.info("🔧 [SOLVER] Stratégie : Quantiles Dynamiques (Calibration Auto)")

        # 1. CHARGEMENT
        candidates = (
            self.db.query(Recipe.id, Recipe.embedding, Recipe.calories)
            .filter(Recipe.embedding is not None, Recipe.minutes <= 120)
            .all()
        )
        ...
        print(
            f"📊 Calibration : PERF > {thresh_perf:.4f} | DISCO entre {thresh_disco_low:.4f} et {thresh_disco_high:.4f}"
        )
        ...
        for item in scored_items:
            ...
            if s >= thresh_perf:
                ...
                item["type"] = (
                    "NEUTRAL"  # Zone grise (40% - 80%) qu'on ignore pour trancher
                )
        ...
        if len(final_selection) < total_slots:
            ...
            remaining_pool.sort(
                key=lambda x: x["score"], reverse=True
            )  # On prend les meilleurs restants
    ...
    def _calculate_similarity(self, embedding_data: str | list | np.ndarray) -> float:
        ...
        """
        if self.user_vector is None or embedding_data is None:
            return 0.5
        try:
