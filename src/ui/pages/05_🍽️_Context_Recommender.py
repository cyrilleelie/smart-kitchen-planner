...
with st.sidebar:
    ...
    selected_meal_label = st.radio("🕒 Moment du repas", list(meal_options.keys()), index=1)
    ...
    selected_season_label = st.selectbox("📅 Saison", list(season_options.keys()), index=0)
...
if launch_btn:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not recommendations:
                ...
                st.success(f"Top 5 Recettes pour **{selected_meal_label}** en **{selected_season_label}**")
                ...
                for idx, rec in enumerate(recommendations):
                    with cols[idx]:
                        ...
                        score_val = rec['score']
                        normalized_score = min(1.0, max(0.0, (score_val - 1) / 4)) # Normalisation 1-5 vers 0-1
                        
                        st.metric(label=f"Rank #{idx+1}", value=f"{score_val}/5")
                        st.progress(normalized_score)
                        
                        st.subheader(rec['name'])
                        st.caption(f"⏱️ {rec['minutes']} min | 🔥 {rec.get('calories', '?')} kcal")
    ...
    st.info("👈 Configurez le contexte dans la barre latérale et cliquez sur 'Générer'.")
