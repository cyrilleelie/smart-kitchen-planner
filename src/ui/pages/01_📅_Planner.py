...
MEAL_TYPES = {"🥐 Petit-déjeuner": 0, "🥗 Déjeuner": 1, "🍲 Dîner": 2, "🍪 Snack": 3}

SEASONS = {"❄️ Hiver": 0, "🌸 Printemps": 1, "☀️ Été": 2, "🍂 Automne": 3}


def main():
    st.title("📅 Générateur de Menu Intelligent")
    st.caption(
        "Un planning adapté à vos goûts, à la saison et au moment de la journée."
    )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if not plan:
                        ...
                        k1.metric(
                            "Moyenne Kcal/Repas",
                            f"{int(stats.get('average_calories', 0))}",
                        )
                        k2.metric(
                            "Score Match IA",
                            f"{float(stats.get('average_match_score', 0)):.2f}/5",
                        )
                        ...
                        CODE_TO_LABEL = {
                            v: k.split(" ")[1] for k, v in MEAL_TYPES.items()
                        }

                        for idx, day in enumerate(sorted(schedule.keys())):
                            with cols[idx]:
                                st.markdown(
                                    f"<div style='text-align:center; background:#e0e7ff; padding:8px; border-radius:8px; margin-bottom:10px;'><b>J{day}</b></div>",
                                ...
                                for i, meal in enumerate(items_of_day):
                                    ...
                                    current_meal_code = selected_meal_codes[
                                        i % len(selected_meal_codes)
                                    ]
                                    ...
                                    with st.container(border=True):
                                        ...
                                        st.markdown(
                                            f"⏱️ {meal['time']} min | 🔥 {int(meal['calories'])}"
                                        )

                                        # Score & Algo Tag
                                        if "Découverte" in meal.get("tags", []):
                                            st.markdown(
                                                "🎲 <span style='color:#6f42c1'><b>Découverte</b></span>",
                                                unsafe_allow_html=True,
                                            )
                                        else:
                                            st.markdown(
                                                f"⭐ **{meal['match_score']:.2f}** / 5"
                                            )
