...
CONTEXT_RULES = {
    "breakfast": ["pancake", "waffle", "toast", "coffee", "cereal", "oat", "yogurt", "smoothie", "croissant", "porridge", "granola", "egg", "bacon"],
    
    # Fusion Déjeuner / Dîner -> Repas Principal
    "main_meal": ["roast", "steak", "stew", "casserole", "pasta", "curry", "lasagna", "gratin", "burger", "rice", "noodle", "pizza", "chicken", "beef", "pork", "fish", "salad", "soup"],
    
    "snack": ["snack", "cookie", "muffin", "bar", "smoothie", "fruit", "chips", "dip", "popcorn", "bites", "nuts", "energy", "cracker", "hummus"],
...
def parse_vector(vec_str):
    if isinstance(vec_str, str): return np.array(eval(vec_str), dtype=np.float32)
    elif isinstance(vec_str, list): return np.array(vec_str, dtype=np.float32)
    elif isinstance(vec_str, np.ndarray): return vec_str
    return np.zeros(384)
...
def train():
    ...
    for index, row in df_raw.iterrows():
        ...
        for _ in range(4):
            ...
            penalty = calculate_context_penalty(row['recipe_obj'], simulated_meal, simulated_season)
            
            target_score = max(1, min(5, base_score + penalty))
            
            context_features = np.array([simulated_meal, simulated_season], dtype=np.float32)
