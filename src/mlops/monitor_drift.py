...
def fetch_prediction_logs(days=7):
    ...
    """
    print("   📡 Récupération + Hydratation des logs (7 derniers jours)...")
...
def monitor():
    ...
    print(
        f"   🔬 Features analysées : {len(features_to_monitor)} dimensions (Embeddings + Context)"
    )
    ...
    # Export Report
    report_path = (
        f"reports/drift_report_{datetime.now().strftime('%Y-%m-%d_%Hh%M')}.html"
    )
